...
class ColorFeatureExtractor:
    ...
    def __init__(self, hist_bins=(32, 8, 8), use_palette=True, use_colorfulness=True, command=None):
        ...
        if self.use_palette:
            self.max_palette_dist = np.sqrt(15 * 255**2)
    ...
    def compute_distance(self, a: dict, b: dict, weights=None) -> float:
        ...
        if self.use_palette and "palette" in a and "palette" in b:
            if a["palette"] is None or b["palette"] is None:
                self._log("Skipping platte since it is empty.", "warning")
            else:
                ...
                normalized = euclid / self.max_palette_dist if self.max_palette_dist else 1.0

                dist += w * normalized
                norm += w

        if self.use_colorfulness and "colorfulness" in a and "colorfulness" in b:
            w = weights.get("colorfulness", 0.2)
            diff = min(self.COLORFULNESS_MAX, abs(a["colorfulness"] - b["colorfulness"])) / self.COLORFULNESS_MAX
            dist += w * diff
            norm += w
    ...
    def distance_to_feature_set(self, query_feat: dict, feature_list: list[dict], weights=None):
        distances = [self.compute_distance(query_feat, f, weights) for f in feature_list]
